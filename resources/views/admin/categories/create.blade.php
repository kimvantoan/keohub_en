...
<div class="bg-white rounded-lg shadow overflow-hidden max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="px-6 py-4">
        @csrf
        <div class="space-y-6">
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700">Tên danh mục <span class="text-red-500">*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
            </div>
