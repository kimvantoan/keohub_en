...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-6 py-4 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Chỉnh sửa bài viết: {{ $article->title }}</h2>
    </div>
    ...
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="px-6 py-4">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="20" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border">{{ old('content', $article->content) }}</textarea>
                </div>
                
                <div>
                    <label for="meta_description" class="block text-sm font-medium text-gray-700">Mô tả ngắn (Meta Description)</label>
                    <textarea name="meta_description" id="meta_description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">{{ old('meta_description', $article->meta_description) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6 bg-gray-50 p-4 rounded-lg border">
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', $article->is_published) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500">
                ...
                <div>
                    ...
                    <div id="thumbnail-preview-container" class="mt-2 mb-2 {{ $article->thumbnail ? '' : 'hidden' }}">
                        <img id="thumbnail-preview" src="{{ $article->thumbnail ? Storage::url($article->thumbnail) : '' }}" alt="Thumbnail preview" class="h-32 object-cover rounded border">
                    </div>
